"""Ed Data Express Title I funding data importer.

Downloads Title I (Part A) allocation data, matches it to existing districts by
NCES ID, and updates their ``title_i_allocation`` and ``title_i_status``
fields.

CLI usage::

    python -m scrapers.nces.title_i [--csv /path/to/local.csv]
"""
...
# Ed Data Express Title I Part A allocations CSV.
# The URL may shift between fiscal years; this is a representative starting point.
DEFAULT_TITLE_I_URL = (
    "https://eddataexpress.ed.gov/download/data-library/"
    "title-i-grants-to-local-educational-agencies.csv"
)


@dataclass
class TitleIRecord:
    """Single Title I allocation record."""
...
class TitleIImporter:
    """Download and import Title I Part A allocation data."""

    def __init__(
        self,
        *,
        url: str = DEFAULT_TITLE_I_URL,
    ...
    async def download(self) -> str:
        """Download the Title I CSV and return its text content."""
        logger.info("Downloading Title I data from %s", self.url)
        ...
        text = resp.text
        logger.info("Downloaded %d chars of Title I CSV", len(text))
        return text
    ...
    def parse_csv(self, csv_text: str) -> list[TitleIRecord]:
        """Parse Title I CSV text into records.
        ...
        records: list[TitleIRecord] = []

        if reader.fieldnames is None:
            raise ValueError("Title I CSV has no header row")
        ...
        col_alloc = (
            _find_col("ALLOCATION")
            or _find_col("AMOUNT")
            or _find_col("TITLE I")
            or _find_col("TITLE_I")
        )
        col_year = _find_col("YEAR") or _find_col("FISCAL")

        if not col_nces:
            logger.warning("Could not identify NCES ID column in Title I CSV")
            return records
        ...
        logger.info("Parsed %d Title I allocation records", len(records))
        return records

    def parse_file(self, path: Path) -> list[TitleIRecord]:
    ...
    async def update_districts(
        ...
        """Update existing district rows with Title I data.

        Sets ``title_i_allocation`` and ``title_i_status = True`` for
        ...
        update_sql = text("""
            UPDATE districts
            SET title_i_allocation = :allocation,
                title_i_status = TRUE,
                updated_at = NOW()
        ...
        logger.info("Updated %d district rows with Title I data", count)
        return count
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Import Ed Data Express Title I Part A allocation data",
    )
    parser.add_argument(
        "--url",
        default=DEFAULT_TITLE_I_URL,
        help="URL to the Title I CSV (default: %(default)s)",
    )
    parser.add_argument(
        "--csv",
        type=Path,
...
async def _main() -> None:
    ...
    importer = TitleIImporter(url=args.url)
    records = await importer.run(csv_path=args.csv, db_session=None)

    print(f"Parsed {len(records)} Title I records")
    if records:
        sample = records[0]
